...
const UploadFile: FC<IUploadFile> = React.forwardRef(
  ({ register, errors }, ref) => {
    /*---- errors ----*/
    const [errorType, setErrorType] = useState<string>('');
    const [errorSize, setErrorSize] = useState<string>('');
    const [errorStyleFakeInput, setErrorStyleFakeInput] = useState<string>(
      ['fakeInput'].join(' '),
    );
    const [errorStyleFileCustom, setErrorStyleFileCustom] = useState<string>(
      ['fileCustom'].join(' '),
    );
    ...
    const fileChangedHandler = (event: any) => {
      ...
      if (file.type.length === 10) {
        ...
        setErrorStyleFileCustom(
          [...errorStyleFileCustom.split(' '), 'errorCustomInput'].join(' '),
        );
        setErrorStyleFakeInput(
          [...errorStyleFakeInput.split(' '), 'errorFakeInput'].join(' '),
        );
      }
    ...
    return (
      <div className="form-personUpload">
        <label className="file">
          ...
          <span className={errorStyleFileCustom}>{nameInput}</span>
        </label>
        {errorSize.length > 1 && (
          <div style={{ color: 'red' }}>{errorSize}</div>
        )}
        {errorType.length > 1 && (
